for s = 1 : n_samples
    ...
    B = zeros(1, n);
    status_A = rand(1, n);
    status_B_1 = (rand(1, n) > 0.33) + 1;
    status_B_2 = (rand(1, n) > 0.33) + 1;
    state_nodes = zeros(1, n);
    status_A(status_A > 0.6) = 3;
    status_A((status_A > 0.3) & (status_A <= 0.6)) = 2;
    status_A(status_A <= 0.3) = 1;
    ...
    for idx = 1 : 2 * n
        ...
        if ~state_sys
            if sum(state_nodes == 0) >= 1 || sum(state_nodes == 2) >= 2
                FB_count = FB_count + 1;
            end
            break;
        end
